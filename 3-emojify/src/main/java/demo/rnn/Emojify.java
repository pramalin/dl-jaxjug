...
//		runModel1();
	...
	public static void runModel2() throws Exception {
		// model
...
//		    		  " prediction: " + X_test.get(i) + Utils.emoji(num));
	...
	public static void runModel1() throws Exception {

		// model
		MultiLayerNetwork model = Utils.Emojify_V1();

        List<String> testAvg = new ArrayList<String>();
        testAvg.add("Morrocan couscous is my favorite dish");
        System.out.println("avg = " + Utils.sentencesToAvg(testAvg));
		...
		INDArray X_train_avgs = Utils.sentencesToAvg(X_train);
		INDArray Y_train_oh = Utils.convertToOneHot2d(Y_train, 5);
		...
		INDArray X_test_avgs = Utils.sentencesToAvg(X_test);
		INDArray Y_test_oh = Utils.convertToOneHot2d(Y_test, 5);

		ListDataSetIterator<DataSet> trainData =
				new ListDataSetIterator<DataSet>(new DataSet(X_train_avgs, Y_train_oh).asList(), 32);
		ListDataSetIterator<DataSet> testData =
				new ListDataSetIterator<DataSet>(new DataSet(X_test_avgs, Y_test_oh).asList());
		...
		// This code allows you to see the mislabeled examples
		INDArray pred = model.output(X_train_avgs);
        ...
        INDArray pred3 = model.output(X_test_avgs);
		double miss = 0;
        System.out.println("Test data evaluation");

		for (int i = 0; i < X_test.size(); i++) {
			int num = Nd4j.argMax(pred3.getRow(i)).getInt(0,0);
			if(num != Y_test.get(i)) {
				miss = miss + 1;
		      System.out.println("Expected emoji: " + Utils.emoji(Y_test.get(i)) +
		    		  " prediction: " + X_test.get(i) + Utils.emoji(num));
		...
		X_test_avgs = Utils.sentencesToAvg(X_test2);
		INDArray pred4 = model.output(X_test_avgs);
		int num4 = Nd4j.argMax(pred4.getRow(0).getColumn(4), 0).getInt(0, 0);
